...
{
    public partial class FRM_TD_PheduyetVB_AttachedFiles : BaseForm_Data
    {
        FRM_TD_PheduyetNhiemvuTuVBCosan frm = null;
        List<TD_Pheduyet_VB_Tepdinhkem> currentList = null;
        public string currentState = "NORMAL";
        public FRM_TD_PheduyetVB_AttachedFiles()
        {
            InitializeComponent();
        }

        private void FRM_TD_PheduyetVB_AttachedFiles_Load(object sender, EventArgs e)
        {
            frm = (FRM_TD_PheduyetNhiemvuTuVBCosan)Application.OpenForms["FRM_TD_PheduyetNhiemvuTuVBCosan"];
        ...
        {
            currentList = new List<TD_Pheduyet_VB_Tepdinhkem>();
            for (int i = 0; i < gridView1.RowCount; i++)
            {
                bool isChecked = (bool)gridView1.GetRowCellValue(i, gridColumn10);
                if (isChecked) currentList.Add(new TD_Pheduyet_VB_Tepdinhkem()
                {
        ...
        private void btn_chontep_Click(object sender, EventArgs e)
        {
            ...
            {
                if (currentList == null) currentList = new List<TD_Pheduyet_VB_Tepdinhkem>();

                foreach (string filePath in openFileDialog1.FileNames)
                {
                    string fileName = DateTime.Now.Ticks.ToString();
                    string uri = Helpers.CreateRequestUrl_UploadFile();
                    uri += "&n=td_pheduyetnhiemvuvb_tepdinhkem&fn=" + fileName;
                    ...
                    {
                        currentList.Add(new TD_Pheduyet_VB_Tepdinhkem()
                        {
                            Filename = fileName,
                            Id = Guid.NewGuid(),
                            IsChecked = true,
